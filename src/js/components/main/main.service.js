(function() {
  ...
  angular
    .module('TeaAppServiceModule', [])
    .service('TeaAppService', TeaAppService);

  function TeaAppService() {

    this.completeTeaList = [
      {
    ...
    this.filterCategories = function(input) {

      var desiredArray = input.map((el) => {
        return el.categories.map((el) => {
          return el;
        });
      }).reduce((a,b) => {
        return a.concat(b);
      ...
      this.categories = Array.from(new Set(desiredArray));
    };

    this.filterCategories(this.completeTeaList)
  }
